...
const getUserBookingsByRange = async(userId, start, end) => {
    ...
    console.log(`KDJFD8, start:\n ${start}, end\n: ${end}`)

    try {
        const aptIdAlias = "aptId";
        const aptIdAs = tableDotField(apmtTable, "id") + " as " + aptIdAlias;

        const aptArr = await db(apmtTable).join(userTable, tableDotField(apmtTable, "user_id"), tableDotField(userTable, "id")).whereBetween(tableDotField(apmtTable, "wish_start_at"), [start, end])
        ...
        for (const apt of aptArr) {
            ...
            const canBeCancelled = moment(apt.wish_start_at).diff(now, "minutes") > cancellableSpanMinute;
